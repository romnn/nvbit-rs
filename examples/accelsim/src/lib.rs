...
lazy_static! {
    ...
    static ref ALREADY_INSTRUMENTED: Mutex<HashSet<nvbit_rs::Function<'static>>> = Mutex::new(HashSet::new());
}
...
fn instrument_function_if_needed<'c, 'f>(
    ...
    // let mut ctx = nvbit_rs::Context::new(ctx);
    // let mut func = nvbit_rs::Function::new(func);

    // let mut related_functions: Vec<&mut nvbit_rs::Function<'_>> =
    let mut related_functions = nvbit_rs::get_related_functions(ctx, func);
    // .iter_mut();
    // .collect();
    ...
    for f in related_functions.iter_mut().chain([func]) {
        ...
        let func_name = nvbit_rs::get_func_name(ctx, &mut f);
        // let func_name: *const ffi::c_char =
        //     unsafe { bindings::nvbit_get_func_name(ctx, f.as_mut_ptr(), false) };

        // let func_name = unsafe { ffi::CStr::from_ptr(func_name).to_string_lossy() };

        // let func_addr: u64 = unsafe { bindings::nvbit_get_func_addr(f.as_mut_ptr()) };
        ...
        //     }
        // };
        // todo: is it okay to clone a function? this does allow concurrent mutable access :(
        if !instrumented_lock.insert(f.clone()) {
            println!(
        ...
        // let instrs = unsafe { rust_nvbit_get_instrs_new(ctx, func) };
        let mut instrs = nvbit_rs::get_instrs(ctx, &mut f);
        ...
        for instr in instrs.iter_mut() {
            ...
            // let instr_ptr: *mut ffi::c_void = unsafe {
            // let instr_ptr: *mut nvbit_sys::bindings::Instr = unsafe {
            //     // let test: *mut nvbit_sys::nvbit::Instr = instr.as_mut() as *mut _;
            //     instr.as_mut_ptr() as *mut nvbit_sys::bindings::Instr
            // };
            // &mut * as *mut nvbit_sys::bindings::Instr };
            // let p: &mut bindings::cuLaunchKernel_params =
            //     unsafe { &mut *(params as *mut bindings::cuLaunchKernel_params) };

            // println!("rust raw instruction pointer: {:p}", instr_ptr);
            ...
            // dbg!(unsafe { bindings::Instr_getOffset(instr_ptr) });
            ...
            inst_args.pchannel_dev = (stats.pchannel_dev as *mut ffi::c_void) as u64;
            ...
            inst_args.preported_dynamic_instr_counter =
                (stats.preported_dynamic_instr_counter as *mut ffi::c_void) as u64;

            inst_args.pstop_report = (stats.pstop_report as *mut ffi::c_void) as u64;
            dbg!(&inst_args);
            println!("pre instrumenting functions");
            inst_args.instrument(instr);
            // unsafe {
            //     // inst_args.instrument(std::ptr::null_mut() as *mut _);
            //     inst_args.instrument(instr);
            //     // inst_args.instrument(unsafe { instr.as_ptr() });
            // };
            println!("post instrumenting functions");
            instr_count += 1;
        }
    }
    println!("instrumented all functions");
}
    ...
    match cbid {
        ...
        | bindings::nvbit_api_cuda_t::API_CUDA_cuLaunchKernel => {
            ...
            if !is_exit {
                ...
                // this works
                instrument_function_if_needed(&mut ctx, &mut pf);

                // unsafe { bindings::nvbit_enable_instrumented(ctx, p.f, true, true) };

                if unsafe { active_region } {
                    nvbit_rs::enable_instrumented(&mut ctx, &mut pf, true, true);
                    // unsafe {
                    //     bindings::nvbit_enable_instrumented(ctx, p.f, true, true);
                    // }
                    unsafe { *stats.pstop_report = false };
                } else {
                    nvbit_rs::enable_instrumented(&mut ctx, &mut pf, false, true);
                    // unsafe {
                ...
                unsafe { common::flush_channel(stats.pchannel_dev as *mut ffi::c_void) };
...
pub extern "C" fn nvbit_at_ctx_init(ctx: bindings::CUcontext) {
    // setup stats here
    stats.init();
...
pub extern "C" fn nvbit_at_ctx_term(ctx: bindings::CUcontext) {
